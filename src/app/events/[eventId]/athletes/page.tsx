...
export default async function AthletesPage({ params }: { params: { eventId: string } }) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-6">
      ...
      {persons.length === 0 ? (
        <p className="text-gray-500">No athletes found. <Link href={`/events/${params.eventId}/import`} className="text-blue-600 hover:underline">Import data</Link> first.</p>
      ) : (
              ...
              {persons.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2">
                    <Link href={`/events/${params.eventId}/athletes/${p.id}`} className="text-blue-600 hover:underline text-xs">
                      View Schedule →
                    </Link>
                  </td>
                </tr>
